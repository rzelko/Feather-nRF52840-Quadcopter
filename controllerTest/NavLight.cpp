...
void NavLight::warning()
{
    repeatBlink(5,
        pixels->Color(220,115, 25),
        pixels->Color(0, 0, 0),
        NAV_WARNING_DELAY * 2);
}

void NavLight::warningUrgent()
{
    repeatBlink(5,
...
// Blink only for battery level warning
void NavLight::repeatBlink(uint8_t times, uint32_t color1, uint32_t color2, uint16_t wait)
{
    for (uint8_t t = 0; t < times; t++)
    {
...
void NavLight::blink(uint32_t color1, uint32_t color2, uint16_t wait)
{
    pixels->fill(color1);
    pixels->show(); 
    delay(wait);
